FROM quay.io/coryodaniel/debian-jre:latest
MAINTAINER pjpires@gmail.com

# Export HTTP & Transport
EXPOSE 9200 9300

# Versions: https://www.elastic.co/downloads/elasticsearch
...
RUN apt-get update && apt-get install -y --no-install-recommends \
  uuid-runtime gosu ca-certificates util-linux curl gnupg openssl \
  && cd /tmp \
  ...
  && adduser --system --disabled-password --no-create-home --group --shell=/sbin/nologin elasticsearch \
